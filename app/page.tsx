...
export default function App() {
  ...
  async function fetchCases() {
    ...
    try {
      ...
      if (!res.ok) {
        setFetchError(`HTTP ${res.status}: ${JSON.stringify(data)}`);
        setCases([]);
      } else {
  ...
  async function handleValuate() {
    ...
    const prompt = `You are an expert luxury watch appraiser specialising in the Swedish and Nordic resale market.
...
Market context: Price for the Swedish/Nordic market in SEK. Reference: Chrono24, Bob's Watches, Stories of Time, Chrono.dk, LuxuryWatches.se.

CRITICAL INSTRUCTION: Respond ONLY with a raw JSON object. No text before or after. Start with { end with }.

{
  ...
  "buyInPrice": number,
  "reasoning": "3-4 sentences on valuation and Nordic market context",
